...
# 🕒 Freshness tuned for HTF
FRESHNESS_LIMIT = {
    "1h": 900,    # 15 min
    "4h": 2700    # 45 min
}

# 📏 ATR multipliers per TF
ATR_MULTIPLIER = {
    "1h": 1.8,
    "4h": 2.5
}
...
def success_rate(direction, df):
    ...
    score += 10 if atr_now > atr_prev else 5
...
send_telegram("🚀 HTF EMA 9–20 + ADX Momentum Screener Started")
    ...
    for symbol in symbols:
        for tf_name, tf_interval in TIMEFRAMES.items():
            try:
                ...
                adx_series = adx(df)
                if not (adx_series.iloc[-2] <= 25 and adx_series.iloc[-1] >= 26):
                    continue
                ...
                if age > FRESHNESS_LIMIT[tf_name]:
                    continue
                ...
                risk = ATR_MULTIPLIER[tf_name] * atr_val
                ...
                message = (
                    f"🔥 <b>EMA 9–20 HTF SIGNAL</b>\n\n"
                    f"🪙 <b>Coin:</b> {symbol}\n"
                    f"📊 <b>Direction:</b> {direction}\n"
                    f"⏱️ <b>Timeframe:</b> {tf_name}\n"
                    f"🎯 <b>Success Rate:</b> {score}%\n"
                    f"💰 <b>Position:</b> {pos} | {lev}\n\n"
